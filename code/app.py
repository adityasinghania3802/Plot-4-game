...
async def start(websocket):
    ...
    try:
        # Send the secret access token to the browser of the first player, where it'll be used for building a "join" and "watch" links.
        event = {
            "type": "init",
            "join": join_key,
            "watch": watch_key,
...
async def replay(websocket, game):
    # Make a copy to avoid an exception if game.moves changes while iteration is in progress. If a move is played while the replay is running, moves will be sent out of order but each move will be sent once and eventually UI will be consistent.
